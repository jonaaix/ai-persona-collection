...
recipes in the UI patterns reference — read it before building UI.
...
Numbers, dates, money and weights go through one shared format module — never formatted by
hand, never with a hardcoded locale. Figures use `tabular-nums`. Times display in the user's
timezone, 24-hour format — never the server's.
...
`border-t border-gray-200 dark:border-white/10 pt-4`. Cancel left of the primary action,
both at default size — never a small size in a modal footer. If the stack ships modal
helpers, use them rather than rebuilding this by hand.